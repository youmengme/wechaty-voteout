...
import { getMustacheView, } from './mustache-view.js';
import { DEFAULT_CONFIG, } from './config.js';
import * as store from './store.js';
export function VoteOut(config) {
    ...
    const talkRepeat = talkers.roomTalker(config.repeat);
    const talkWarn = talkers.roomTalker(config.warn);
    const talkKick = talkers.messageTalker(config.kick);
    return function VoteOutPlugin(wechaty) {
        ...
        wechaty.on('message', async (message) => {
            ...
            log.verbose('WechatyPluginContrib', 'VoteOut() on(message) payload for votee %s is %s', votee, JSON.stringify(payload));
            /**
             * The voter has already voted the votee before
             */
            if (payload.downIdList.includes(voter.id)
                || payload.upIdList.includes(voter.id)) {
                const view = await getMustacheView(config, payload, room, votee);
